...
// basic idea: 
// * we can't define a specialized template after it is used, but we can add a new specialization to a static const member variable
// * extern C context doesn't allow templates (even for friends), so we have to wrap it in a non-template struct.
