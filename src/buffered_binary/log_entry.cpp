...
log_entry::log_entry(typegrind::logger::entry_alloc const& info)
    ...
    , ownerTargetName_null(
          info.ownershipScope ? reinterpret_cast<uint64_t>(info.ownershipScope->targetName) : 0)
    , ownerCustomName_null(
          info.ownershipScope ? reinterpret_cast<uint64_t>(info.ownershipScope->customName) : 0)
