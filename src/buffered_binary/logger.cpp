...
namespace typegrind
{
    namespace buffered_binary
    {
        ...
        {
            std::string directory(".typegrind-" + std::to_string(std::chrono::high_resolution_clock::now().time_since_epoch().count()));
            // using std::experimntal instead of boost to avoid a library dependency for every project using the logger
