...
loader::loader(std::string directory) {
  ...
  while (loadStrings(directory, loadingThread, strings) &&
...
bool loader::loadStrings(std::string const& directory, unsigned threadId,
                         std::unordered_map<uint64_t, std::string>& strings) {
...
bool loader::loadRun(std::string const& directory, unsigned threadId,
