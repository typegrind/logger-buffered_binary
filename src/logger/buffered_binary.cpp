...
entry_alloc::entry_alloc(entry_type entryType, const char* typeStr, const char* canonicalTypeStr,
...
method_scope::method_scope(const char* targetName, const char* locationStr, const char* customName,
                           flags_type flags)
    : targetName(targetName), locationStr(locationStr), customName(customName), flags(flags) {}
}
}
